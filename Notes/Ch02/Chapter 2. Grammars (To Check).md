...
ВНИМАНИЕ!
не хватает именно **мини-примеров**:

- dangling `else`;
- отличие синтаксической ошибки от семантической;
- пример, где код синтаксически корректен, но не проходит type checking;
- пример Unicode escape, который ломает токенизацию;
- пример `List<List<String>>`, где важно понимание токенов `>`.
